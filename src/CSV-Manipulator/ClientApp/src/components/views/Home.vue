      ...
      <form action="#" method="post" enctype="multipart/form-data">
        <div>
          ...
          <div>
            ...
            <input type="checkbox" id="HasHeaders" @change="CheckHeaders" checked />
            <label for="HasHeaders">Has Headers?</label>
          </div>
        </div>
      </form>
      <!-- Parse file -->
      <button class="btn btn-primary" @click="ParseFile(LoadedFile, ParsedFile)">Parse CSV</button>
    </template>
    <template v-else>
      <h4>
        Now change the data, delete or add columns!
        <br />When it's done,
          ...
          <thead>
            <tr>
              ...
              <th v-for="(header, index) in Headers" :key="index" scope="col">
                <input class="text-center" type="text" v-model="Headers[index]" />
          ...
          <tbody>
            ...
            <tr v-for="(data, index) in ParsedFile" :key="index">
              <td v-for="(element, i) in data" :key="i">
                <input class="text-center" type="text" v-model="ParsedFile[index][i]" />
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="container">
        <input
          ...
          required
        />
        <button class="btn btn-success" @click="SaveJSON(JSONFile)">Save</button>
      </div>
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    CheckHeaders: function() {
      let vm = this as any;
      let element = document.getElementById("HasHeaders") as HTMLInputElement;
